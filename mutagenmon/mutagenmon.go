...
func (self *MutagenMon) CheckStates(ctx context.Context, states map[string]*synchronization.State) error {
	var bad int
	for id, new := range states {
		old, ok := self.states[id]
		if !ok {
			item := systray.AddMenuItem(fmt.Sprintf("%s:%s", new.Session.Beta.Host, new.Session.Beta.Path), "")
			self.menu[id] = item
			self.UpdateMenuItem(self.menu[id], new)
			self.states[id] = new
			if isBad(new) {
				bad++
			}
		...
		if old.Status != new.Status || len(old.Conflicts) != len(new.Conflicts) {
			self.UpdateMenuItem(self.menu[id], new)
			self.states[id] = new
		}
		if isBad(new) {
